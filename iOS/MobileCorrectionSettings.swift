...
extension MobileAISettings {
    ...
    var correctionAvailability: String? {
        if !hasKey { return "Add your Ollama API key in AI settings." }
        if correctionModel.isEmpty || !Self.isFlash(correctionModel) { return "Choose a Flash model for speech correction." }
    ...
    func correctionClient() throws -> OllamaProvider {
        ...
        guard !key.isEmpty else { throw RecordingError.message("Add your Ollama API key in AI settings.") }
        let configuration = URLSessionConfiguration.ephemeral
        configuration.timeoutIntervalForResource = 12
        return OllamaProvider(model: correctionModel, baseURL: OllamaCloudCatalog.baseURL, apiKey: key,
