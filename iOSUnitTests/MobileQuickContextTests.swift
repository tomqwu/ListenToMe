...
final class MobileQuickContextTests: XCTestCase {
    func testKeepAdvancesMemoryAndOnlyUnreadSpeechIsSentNext() throws {
        ...
        pieces = MobileQuickContext.pieces(notes: "", segments: [first, second])
        let next = try XCTUnwrap(context.batch(pieces, summary: ""))
        let input = try JSONDecoder().decode(MobileQuickContext.Input.self, from: Data(next.request.messages[0].content.utf8))
    ...
    func testPartialIsExcludedAndCorrectionAndRemovalAreExplicit() throws {
        ...
        let pieces = MobileQuickContext.pieces(notes: "", segments: [segment, partial])
        XCTAssertEqual(pieces.count, 1)
        let batch = try XCTUnwrap(context.batch(pieces, summary: ""))
        let corrected = segment.withCorrection("Peter confirms.", model: "test-flash")
        let changed = MobileQuickContext.pieces(notes: "", segments: [corrected])
    ...
    func testLongChineseInputIsBatchedWithoutDroppingTextOrExceedingBudget() throws {
        ...
        let pieces = MobileQuickContext.pieces(notes: "", segments: [segment])
        var context = MobileQuickContext()
        var recovered = ""
        var batches = 0
    ...
    func testEvaluationContainsTypedReviewRecommendationsAndRejectsInvalidConfidence() throws {
        ...
        let result = try MobileQuickDecision.parse(response)
        ...
        XCTAssertThrowsError(try MobileQuickDecision.parse(response.replacingOccurrences(of: "high", with: "certain")))
        XCTAssertThrowsError(try MobileQuickDecision.parse(response.replacingOccurrences(of: "deep", with: "publish")))
    }
    ...
    func testStrictDecisionContractKeepsReasoningAndMalformedOutputOffScreen() throws {
        ...
        for prefix in ["", "<think>internal reasoning</think>\n", "Internal preamble\n```json\n"] {
            XCTAssertEqual(try MobileQuickDecision.parse(prefix + valid).summary, "- Monday delivery")
        }
        XCTAssertNil(try MobileQuickDecision.parse(#"{"reviews":[],"action":"keep","context":"Proposal only","bullets":[]}"#).summary)
        for invalid in [valid + " trailing", String(valid.dropLast()),
            ...
            XCTAssertThrowsError(try MobileQuickDecision.parse(invalid))
        }
    }
}
