...
final class MobileSummarySchedulerTests: XCTestCase {
    private let start = ContinuousClock.now
    private var ready: MobileSummaryScheduler.Snapshot {
    ...
    func testTranscriptEventsBatchAtOriginalDeadlineAndNeverEvaluateSilence() {
        var scheduler = MobileSummaryScheduler(interval: .seconds(5))
    ...
    func testEvaluationPlansPublicationAndRecommendationsWithoutAutomaticDeep() throws {
        let evaluation = try MobileQuickDecision.parse(#"{"action":"publish","context":"Decision","bullets":["Monday"],"reviews":[]}"#)
        XCTAssertEqual(MobileSummaryScheduler.actions(for: evaluation), [.publishQuick("- Monday")])
        let keep = MobileQuickDecision(action: "keep", context: "Options", bullets: [],
            reviews: [.init(mode: "deep", confidence: "high", reason: "Unresolved tradeoff")])
        XCTAssertEqual(MobileSummaryScheduler.actions(for: keep), [.keepQuick, .suggestReview(keep.reviews[0])])
    }
}
